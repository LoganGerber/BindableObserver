...
var BindableObserver = /** @class */ (function () {
    ...
    function BindableObserver(eventEmitter) {
         ...
         * member.
         */
        this._inverseSymbolMap = new Map();
        /**
         ...
         */
        this._uniqueNameMap = new Map();
        /**
         * Mapping of Events to user-defined symbols.
         */
        this._overrideEventSymbolMap = new Map();
        /**
         ...
         */
        this._throwNonUniqueNameErrors = true;
        this._relays = [];
        this._idCache = [];
        ...
        this._emitEventSymbol = this.getOrCreateEventSymbol(EmitEvent_1.EmitEvent);
        if (eventEmitter) {
    ...
    Object.defineProperty(BindableObserver.prototype, "cacheLimit", {
        ...
        set: function (limit) {
            ...
            var oldLimit = this._cacheLimit;
            this._cacheLimit = limit;
            var idCacheOverflow = this._idCache.length - limit;
    ...
    BindableObserver.prototype.setEmitter = function (eventEmitter) {
        ...
        if (this._emitEmitterChangedEvents && this._emitter) {
            this.emit(new EmitterChangedEvent_1.EmitterChangedEvent(this, this._emitter, newEmitter));
        }
    ...
    BindableObserver.prototype.emit = function (event) {
        ...
        var ret = this._emitter.emit(eventSymbol, event);
        if (this._emitEmitEvents) {
            var invokeEvent = new EmitEvent_1.EmitEvent(event);
            this._emitter.emit(this.getOrCreateEventSymbol(invokeEvent), invokeEvent);
        }
        return ret;
    };
    /**
    ...
    BindableObserver.prototype.removeAllListeners = function (event) {
        ...
        if (event) {
            ...
            if (eventName === this._emitEventSymbol) {
                ...
                listeners = this._emitter.listeners(eventName);
                // filter out the listeners used for binding observers
                for (var _i = 0, _a = this._relays; _i < _a.length; _i++) {
                    var relay = _a[_i];
            ...
            for (var _d = 0, _e = this._emitter.eventNames(); _d < _e.length; _d++) {
                var event_1 = _e[_d];
                var eventType = void 0;
                if (typeof event_1 === "symbol" && (eventType = this._inverseSymbolMap.get(event_1))) {
    ...
    BindableObserver.prototype.bind = function (relay) {
        if (this._relays.find(function (element) { return element.relay === relay; })) {
            return;
        }
    ...
    BindableObserver.prototype.checkBinding = function (relay) {
        return this._relays.find(function (e) { return e.relay === relay; }) !== undefined;
    };
    /**
    ...
    BindableObserver.prototype.unbind = function (relay) {
        var foundIndex = this._relays.findIndex(function (element) { return element.relay === relay; });
        if (foundIndex === -1) {
            return;
        }
        var found = this._relays[foundIndex];
    ...
    BindableObserver.prototype.getOrCreateEventSymbol = function (event) {
        ...
        var overrideSymbol = this._overrideEventSymbolMap.get(constructor);
        if (overrideSymbol) {
            return overrideSymbol;
        }
        var sym = this._eventSymbolMap.get(constructor);
        if (!sym) {
    ...
    BindableObserver.prototype.registerEvent = function (event, forceUniqueSymbol) {
        ...
        var hasSymbol = this._eventSymbolMap.has(constructor);
        var hasOverrideSymbol = this._overrideEventSymbolMap.has(constructor);
        ...
        if (forceUniqueSymbol) {
            ...
            if (hasSymbol) {
                var oldSym = this._eventSymbolMap.get(constructor);
            ...
            if (this._uniqueNameMap.has(name)) {
                if (this.throwOnNonUniqueEventName) {
                    throw new NonUniqueNameRegisteredError_1.NonUniqueNameRegisteredError(name, this._uniqueNameMap.get(name), constructor);
                }
                else {
                    return false;
                }
            ...
            if (hasOverrideSymbol) {
                var oldSym = this._overrideEventSymbolMap.get(constructor);
